...
function main(){
    const channels = [new SMSChannel(), new PushNotificationChannel()]
    const alertNotification = new AlertNotification(channels)    
    alertNotification.notify('Se ha detectado acciones maliciosas en tu sesion.')
    alertNotification.addChannel(new EmailChannel())
    alertNotification.notify('Se ha detectado acciones maliciosas en tu sesion.')
}
main()