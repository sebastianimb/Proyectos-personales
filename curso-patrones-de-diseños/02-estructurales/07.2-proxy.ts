...
class DocumentProxy implements Document {
  ...
  private mustHasRole: Array<string>

  constructor(document: Document, mustHasRole: Array<string> = []){
    this.document = document
    this.mustHasRole= mustHasRole
  }

  displayContent(user: User): void {
    if (this.mustHasRole.includes(user.getRole())) {
      this.document.displayContent(user)
      return
    }
    else{ console.log(`%cAcceso denegado. ${user.getName()}, no tienes permisos suficientes para ver este documento.`, COLORS.red);
    }
  }
}
